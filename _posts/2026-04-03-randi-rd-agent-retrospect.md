...
tags: [FastAPI, ChromaDB, LangGraph, Gemini, Docker, RAG, 국가RD, 기업마당, multilingual-e5-base, Spring Boot, 검색평가, MRR, nDCG, 부트스트랩]
...
## 2026-07-30 업데이트 — 검색 품질을 측정하고 개선했다

4월 회고에는 숙제가 하나 남아 있었다. Step 2 유사 RFP 탐색이 "실제로 관련 문서를 상위에 올리는가"를 측정한 적이 없다는 것이다. 7월에 평가 파이프라인과 벤치마크를 추가하고 재정렬 로직을 개선했다. 그 기록을 남긴다.

### 홀드아웃 벤치마크 설계

질문-정답 문서 쌍 30개를 수동 라벨링해 홀드아웃 세트(학습·튜닝에 쓰지 않고 평가 전용으로 떼어둔 데이터)로 만들었다. 코퍼스는 16개 기관의 문서 1,620건, 청크 2,204개다. 평가는 문서 단위로 했다. 한 문서에서 청크 여러 개가 검색되면 최고 점수만 남긴다. 청크 단위로 세면 같은 문서가 중복 집계되어 지표가 부풀기 때문이다.

### 재정렬 개선

개선은 두 가지다. 첫째, 후보 검색 범위를 10배로 넓힌 뒤 본문 점수와 제목 의미 점수를 결합했다. 둘째, 120자 이하 짧은 쿼리에는 정규화한 제목 어휘 점수를 5% 가중치로 더했다. 공고 제목의 행정식 문자열 변형(연도·차수 표기 차이)에 대응하기 위해서다.
...
|------|--------|--------|
| Success@1 | 56.7% | 76.7% |
| Success@5 | 76.7% | 100.0% |
| MRR | 0.6664 | 0.8733 |
| nDCG@5 | 0.6862 | 0.9057 |

Success@k는 상위 k개 안에 정답 문서가 포함된 비율이다. MRR은 정답이 등장한 순위의 역수 평균, nDCG@5는 상위 5개의 순서 품질 지표다.

우연한 개선인지 확인하려고 퍼센타일 부트스트랩(복원추출로 표본을 재구성해 불확실성을 추정하는 방법)으로 10,000회 재표본했다. Success@5 개선 폭의 95% 신뢰구간은 +10.0~+40.0%p로 0을 포함하지 않는다. 다만 100%는 고정된 30문항 세트에서의 수치다. 문항을 늘리거나 라벨을 교차 검토하면 내려갈 수 있고, 이 한계는 README에도 명시했다.

4월의 "결과가 빈약했다"는 서술은 감이었다. 지금은 측정값과 그 한계를 같이 말할 수 있다. 측정 없이는 개선을 주장할 수 없다는 게 이 작업의 결론이다.
