---
title: "M&A 밸류에이션 기초 — EV/EBITDA 11.6배와 7.2배, 배수를 가르는 건 업종이 아니다"
date: 2026-08-21
categories: [business-economy]
tags: ["M&A", "밸류에이션", "EV/EBITDA", "DCF", "기업가치평가", "기업금융"]
...
## EV/EBITDA를 기본값으로 쓰는 이유

EV(Enterprise Value, 기업가치)는 시가총액에 순차입금을 더한 값이다. 주주 몫만 보는 시가총액과 달리 채권자 몫까지 담는다. EBITDA는 이자·세금·감가상각비를 빼기 전 영업이익이다.

분자와 분모가 모두 자본구조·세율 이전 단계다. 차입이 많은 회사와 무차입 회사를 같은 자로 잴 수 있다. PER은 순이익이 이자비용을 이미 반영해 이게 안 된다. 인수 후 자본구조를 새로 짤 인수자에게는 쓸모가 적다.
...
| 상장 기술기업 중 흑자 기업 (2026.1) | 24.48배 | [NYU Stern](https://pages.stern.nyu.edu/~adamodar/New_Home_Page/datafile/vebitda.html) |

소프트웨어 중간값은 2023~2024년 17~22배에서 2025년 하반기 26.1배로 올라섰다([Aventis Advisors](https://aventis-advisors.com/software-valuation-multiples/)). 제조업 6.7배와 소프트웨어 19배는 3배 차이다. 이걸 "업종 프리미엄"으로만 읽으면 다음 질문에서 막힌다.
...
DCF는 향후 잉여현금흐름을 WACC(가중평균자본비용)로 할인해 합산한다. 시장이 얼마를 지불하는지가 아니라 이 회사가 앞으로 얼마를 벌지를 잰다.

문제는 무게중심이다. 터미널밸류(예측기간 이후의 영구가치)가 전체 기업가치의 60~80%를 차지한다([Terminal Value Formula](https://soferadvisors.com/insights/blog/terminal-value-formula-how-to-calculate-it-in-a-dcf/)). WACC가 보통 7~10%, 영구성장률이 1.5~3.5%라 분모의 두 값이 가깝다. 영구성장률을 0.5%p만 움직여도 평가액이 15~20% 흔들린다. 민감도 분석이 부록이 아니라 본문인 이유다([DCF 민감도 분석 가이드](https://www.fe.training/free-resources/valuation/dcf-sensitizing-for-key-variables/)).

DCF의 산출물은 숫자 하나가 아니라 밴드다. 배수 밴드와 겹치는 구간이 협상에서 방어 가능한 범위다.
...
먼저 정할 건 방법이 아니라 모집단이다. 규모 구간, 상장 여부, 흑자 여부, 인수자 유형을 맞춘 뒤에야 11.6배와 7.2배가 같은 언어가 된다. 배수는 시장이 지금 지불하는 값, DCF는 내가 벌 수 있다고 보는 값이다. 둘이 갈리면 가운데를 취할 게 아니라 어느 가정이 틀렸는지 찾아야 한다.

다음 단계는 관심 업종의 최근 공시 거래 3~5건에서 EV와 조정 EBITDA를 직접 계산해보는 것이다. 공표된 배수와 값이 다르면 그 차이가 곧 그 시장의 정상화 관행이다.
