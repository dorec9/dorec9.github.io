...
6. 아래 `주제별 시각 자료 판단`에 따라 글의 핵심을 설명하거나 증명하는 시각 자료를 만든다.
7. `_data/topic-history.yml`에 slug, date, title, keywords를 추가한다. 회고 글은 repo와 SHA도 기록한다.
8. 대기열 항목을 `status: published`로 바꾸고 `post`에 파일명을 기록한다.
9. `python scripts/validate_post.py <포스트 경로>`와 `bundle exec jekyll build`를 실행한다. Ruby/Bundler가 없으면 그 사실을 기록하되, 포스트 검증은 생략하지 않는다.
10. 검증 실패는 두 번까지 수정한다. 해결하지 못하면 `status: failed`와 `failure_reason`을 기록하고, 글 파일과 히스토리 변경은 커밋하지 않는다.
11. 검증 성공 시 포스트, 히스토리, 대기열과 필요한 이미지 파일만 명시적으로 stage한다. `글 발행: <제목>`으로 커밋하고 `git push origin main`을 실행한다.
12. push 후 원격 커밋과 GitHub Pages 배포 상태를 확인한다.
...
글을 쓰기 전에 독자가 가장 이해하기 어려운 관계가 무엇인지 먼저 정한다. 모든 글에 같은 이미지나 장식용 대표 이미지를 넣지 않는다.

- 과정·순서·분기·시스템 관계가 핵심이면 Mermaid 흐름도나 아키텍처를 사용한다.
- 수치의 변화·비교·분포가 핵심이면 출처가 추적되는 실제 데이터로 Chart.js 그래프를 만든다.
- 선택 기준·평가 항목·옵션 비교가 핵심이면 표, 체크리스트, 의사결정 매트릭스를 만든다.
- 프로젝트 결과나 제품 사용법이 핵심이면 비밀정보를 제거한 실제 화면, 전후 비교, 코드 결과를 사용한다.
- 현장·장비·행사 자체가 근거일 때만 직접 촬영했거나 사용권이 확인된 사진을 사용한다.
- 수식이나 알고리즘이 핵심이면 KaTeX 수식과 작은 계산 예시를 함께 사용한다.

시각 자료는 본문 핵심 주장과 직접 연결되어야 한다. 본문에서 자료를 해석하고 독자가 무엇을 봐야 하는지 설명한다. 이미지에는 구체적인 대체 텍스트와 캡션을 붙인다. 외부 데이터·화면·사진을 사용하면 자료 바로 아래에 출처와 기준 연도를 적는다. 직접 만든 도식은 `작성자 정리`로 표시할 수 있다.

다음 자료는 사용하지 않는다.

- 내용과 무관한 스톡 사진과 분위기용 AI 이미지
- 출처나 기준 연도가 없는 수치 그래프
- 본문을 그대로 상자에 옮긴 장식용 다이어그램
- 읽을 수 없게 축소된 화면 전체 캡처
- 실제 측정값처럼 보이게 만든 예시 데이터

발행 전 시각 자료만 보아도 글의 핵심 관계 하나를 이해할 수 있는지 확인한다. 포스트에는 Mermaid, Chart.js, 의미 있는 표, 출처가 있는 이미지 중 최소 하나가 있어야 한다.
